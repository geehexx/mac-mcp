...
- [ ] Requirement crystal clear (**use ask_followup_question immediately if not**)
...
**Scope creep prevention**: If todo list grows >50% beyond original, STOP and ask user for scope change approval.
...
- Before using write_to_file or replace_in_file
...
3. **Use ask_followup_question if:**
...
- Validate consistency with `.clinerules/rules/reference/code-patterns.md`
- **Use ask_followup_question to confirm if multiple valid options**
- Document new patterns, get user approval if significant
...
**When to use ask_followup_question during review**: Complex architectural decisions, trade-offs, unclear requirements, high-risk changes, ANY uncertainty.
