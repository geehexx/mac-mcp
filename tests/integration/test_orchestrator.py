...
from mac_mcp.core.decomposer import GoalDecomposer, TaskDAG, TaskSpec
...
class TestGoalWorkflowIntegration:
    ...
    async def test_dependency_blocking(self) -> None:
        ...
        # Complete task 1
        await orchestrator.complete_task("agent1", "task_1", {})
    ...
    async def test_goal_failure_propagation(self) -> None:
        ...
        assert updated_goal is not None
        task_states = {t.id: t.state.value for t in orchestrator._tasks.values() if t.goal_id == "goal_fail"}
        progress = updated_goal.calculate_progress(task_states)
